...
export const createCheckOutSession = async (req, res) => {
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      return res.status(400).json({ error: "Invalid or no products" });
    }
    let totalAmount = 0;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel__url: `${process.env.CLIENT_URL}/purchase-cancel`,
      discounts: coupon
        ? [
            {
...
export const checkOutSuccess = async (req, res) => {
  try {
    ...
    console.log("error in checkout-sucess controller",error.message);
    return res.status(500).json({error:"Server Error"})
  }
};
...
async function createNewCoupon(userId) {
  const newCoupon = new Coupon({
    code: "COUPON" + Math.random().toString().substring(2, 8).toUpperCase(),
    discountPercentage: 10,
    expirationDate: new Date(Date.now() + 60 * 60 * 24 * 30 * 1000),
    userId: userId,
  });
